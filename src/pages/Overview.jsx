          ...
          <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {lectures.map((lecture, idx) => {
                const rotations = ["-rotate-1", "rotate-1 mt-2", "-rotate-2 mt-1"];
        ...
        <div className="sticky bottom-0 left-0 w-full p-6 md:p-8 bg-gradient-to-t from-background via-background/95 to-transparent flex justify-center z-20">
          <div className="w-full max-w-3xl relative mt-4">
            ...
            <form 
              ...
              className="bg-[#fdfbf7] border-4 border-primary rounded-xl shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] flex items-center p-3 transform hover:-translate-y-1 transition-transform duration-200"
            >
              <span className="material-symbols-outlined text-primary ml-3 mr-4 text-3xl">search</span>
              <input 
                className="flex-1 bg-transparent border-none outline-none font-body-lg text-xl text-primary placeholder:text-on-surface-variant/60 focus:ring-0" 
              ...
              {activeStream.isStreaming ? (
                <button 
                  ...
                  className="bg-error text-white border-2 border-error w-12 h-12 rounded-lg flex items-center justify-center hover:bg-error/80 transition-colors hover:scale-105 active:scale-95"
                >
                  <span className="material-symbols-outlined text-xl font-bold">stop</span>
                </button>
              ) : (
                <button 
                  type="submit"
                  disabled={!inputText.trim()}
                  className="bg-secondary-container border-2 border-primary w-12 h-12 rounded-lg flex items-center justify-center hover:bg-secondary-fixed transition-colors hover:scale-105 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
                >
